...
# general
parser.add_argument('--model-prefix', default='work_dirs/tiff_transface++_s/model.pt', help='path to load model.')
...
class Embedding(object):
    def __init__(self, prefix, data_shape, batch_size=1):
        ...
        resnet = get_model_tiff(args.network, dropout=0, fp16=False).cuda()
    ...
    def forward_db(self, batch_data):
        ...
        len_file_bytes = _bytes_to_int32(_image_to_bytes(imgs[0], format="tiff")).shape[0] 
        img_file_bytes = torch.zeros(imgs.shape[0], len_file_bytes).cuda() 
        for index in range(imgs.shape[0]):
            each_file_bytes = _bytes_to_int32(_image_to_bytes(imgs[index], format="tiff"))  
