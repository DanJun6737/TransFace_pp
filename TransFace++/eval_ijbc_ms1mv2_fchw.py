...
from backbones import get_model_fchw 
...
# general
parser.add_argument('--model-prefix', default='work_dirs/fchw_transface++_s/model.pt', help='path to load model.')
...
class Embedding(object):
    def __init__(self, prefix, data_shape, batch_size=1):
        ...
        resnet = get_model_fchw(args.network, dropout=0, fp16=False).cuda()
    ...
    def forward_db(self, batch_data):
        ...
        len_file_bytes = (imgs[0]* 255).byte().to(dtype=torch.int32).reshape(-1).shape[0] 
        img_file_bytes = torch.zeros(imgs.shape[0], len_file_bytes).cuda()     
            
        for index in range(imgs.shape[0]):
            each_file_bytes = (imgs[index]* 255).byte().to(dtype=torch.int32).reshape(-1)  
