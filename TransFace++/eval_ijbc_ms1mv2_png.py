...
from backbones import get_model_png 
...
# general
parser.add_argument('--model-prefix', default='work_dirs/png_transface++_s/model.pt', help='path to load model.')
parser.add_argument('--image-path', default='/mnt/workspace/ijb/IJBC', type=str, help='') 

parser.add_argument('--result-dir', default='.', type=str, help='')
parser.add_argument('--batch-size', default=128, type=int, help='')
parser.add_argument('--network', default='vit_s_dp005_mask_0', type=str, help='')
...
class Embedding(object):
    def __init__(self, prefix, data_shape, batch_size=1):
        ...
        resnet = get_model_png(args.network, dropout=0, fp16=False).cuda()
    ...
    def forward_db(self, batch_data):
        ...
        len_file_bytes = _bytes_to_int32(_image_to_bytes(imgs[0], format="png", compress_level = 0)).shape[0]
        img_file_bytes = torch.zeros(imgs.shape[0], len_file_bytes).cuda()  
        
        for index in range(imgs.shape[0]):
            each_file_bytes = _bytes_to_int32(_image_to_bytes(imgs[index], format="png", compress_level = 0))
                    
            img_file_bytes[index] = each_file_bytes  
        
        feat, weight, local_patch_entropy = self.model(img_file_bytes, phase="infer")
