...
from backbones import get_model_tiff
...
def main(args):
    ...
    for epoch in range(start_epoch, cfg.num_epoch):
        ...
        for _, (img, local_labels) in enumerate(train_loader):    
            ...
            img_original= ( (img*0.5) + 0.5 )  
            len_file_bytes = _bytes_to_int32(_image_to_bytes(img_original[0], format="tiff")).shape[0] 
            img_file_bytes = torch.zeros(img_original.shape[0], len_file_bytes).cuda()     
            for index in range(img_original.shape[0]):
                each_file_bytes = _bytes_to_int32(_image_to_bytes(img_original[index], format="tiff"))  
                
                img_file_bytes[index] = each_file_bytes  ## tiff
            
            local_embeddings, weight, local_patch_entropy = backbone(img_file_bytes)
...
if __name__ == "__main__":
    ...
    parser.add_argument("--config", type=str, help="py config file", default='configs/tiff_transface++.py')
