...
def main(args):
    ...
    for epoch in range(start_epoch, cfg.num_epoch):
        ...
        for _, (img, local_labels) in enumerate(train_loader):    
            ...
            len_file_bytes = (img_original[0]* 255).byte().to(dtype=torch.int32).permute(1, 2, 0).reshape(-1).shape[0] 
            img_file_bytes = torch.zeros(img_original.shape[0], len_file_bytes).cuda()     
            
            for index in range(img_original.shape[0]):
                each_file_bytes = (img_original[index]* 255).byte().to(dtype=torch.int32).permute(1, 2, 0).reshape(-1) 
...
if __name__ == "__main__":
    ...
    parser.add_argument("--config", type=str, help="py config file", default='configs/fHWC_transface++.py')
